...
- 本地 `pnpm dev` 默认连接本地 SQLite，避免前端开发误写远端库
- 本地需要核对远端时使用 `pnpm dev:daily`；只有明确需要时使用 `pnpm dev:prod`
- 云端运行时继续使用 canonical Turso 变量：`TURSO_DATABASE_URL` + `TURSO_AUTH_TOKEN`
- Prisma CLI 仍然使用本地 SQLite 生成迁移，避免把 `migrate` 直接打到 Turso
- `next dev` 直启会在 development 环境忽略 canonical Turso，远端连接必须通过项目脚本显式选择
...
- `pnpm db:seed:research-mocks` 默认只写本地 SQLite；远端 mock seed 需要同时传 `--target=<daily|production>` 并设置 `COIN_HUB_ALLOW_REMOTE_MOCK_SEED=1`
